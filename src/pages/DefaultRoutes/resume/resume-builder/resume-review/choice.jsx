...
export default function Choice() {
  const data = [
    {
      icon: <BiCommentDetail className="w-5 h-5 md:w-8 md:h-8 text-secondary-600" />,
    ...
    {
      icon: <MdAutoGraph className="w-5 h-5 md:w-8 md:h-8 text-secondary-600" />,
  ...
  return (
    <section className="bg-primary-600 ">
      <div className="w-full mx-auto max-w-screen-2xl flex flex-col gap-14 md:gap-28 py-28 px-7 lg:px-16">
        ...
        <div className="flex justify-center">
          <Link className="text-white bg-secondary-600 px-12 py-6 rounded-">Review Your Resume Now</Link>
        </div>
      </div>
    </section>
  );
...
export function ChoiceCard({ item }) {
  ...
  return (
    ...
    >
      <div className={`md:h-full rounded-2xl ${onHover ? "bg-primary-400/90" : "bg-primary-600"} w-full items-center flex flex-col gap-6 md-gap-8 pt-16 pb-14 md:pt-12 md:pb-8 px-4 text-white`}>
        ...
        <div className="w-11/12 md:w-full flex flex-col gap-1 md:gap-8 items-center">
          ...
          <p className="text-center w-8/12 md:w-full box-border">{item.content}</p>
        </div>
      </div>
    </div>
  );
